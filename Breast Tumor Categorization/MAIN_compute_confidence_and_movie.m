...
%Choos the folder and segmentation model you'd like to analyse-------------

for i = 1:size(folders,2)
for ii = 1:size(models,2)
    folder = folders{1,i};  model = models{1,ii};

    info = preparing_feature_tables(folder, model,path);
    disp(info)

    info = predicting_confidence(folder, model,path);
    disp(info)

    info = make_movie_confidence(folder, model,path);
    disp(info)
end
end